 ...
 */
public final class JOptionMoverPerosna_Persona extends java.awt.Dialog implements ActionListener
{

    Nodo raizPersona = null;
    Nodo raiz = null;
    String seleccionGrupo=null;
    String grupoAMover = null;

    static String[] personasAMover;
    int defaultY = 316;
     ...
     * @param raizPersona
     * @param raiz
     * @param seleccionadoGrupo
     */
    public JOptionMoverPerosna_Persona(java.awt.Frame parent, boolean modal, Nodo raizPersona,Nodo raiz, String seleccionadoGrupo)
    {
        super(parent, modal);
        initComponents();

        this.raizPersona = raizPersona;
        this.raiz = raiz;
        this.seleccionGrupo=seleccionadoGrupo;

        Nodo aux = this.raizPersona;
    ...
    private void initComponents()
    {
        ...
        btnSiguiente.setIcon(new javax.swing.ImageIcon(getClass().getResource("/iconos/derecha.png"))); // NOI18N
        ...
        jLInformacion.setText("Perona(s) a mover");
    ...
    {//GEN-HEADEREND:event_btnSiguienteActionPerformed
        ...
        difuminadoSalir();
            JOptionMoverPerosna_Grupo sigiente = new JOptionMoverPerosna_Grupo(this.paren, true, raiz, seleccionGrupo,personasAMover);
            sigiente.setVisible(true);
        if (sigiente.isValida())
        {
            grupoAMover = sigiente.getNombreGrupo();
            valida=true;
            setVisible(false);
            dispose();
        }
    ...
    {
        personasAMover = new String[0];
        System.out.println(camposIzquierda.length);
        for (int i = 0; i <= camposIzquierda.length - 1; i++)
        {
            if (camposIzquierda[i].isSelected() == true)
            {
                String tmp[] = new String[personasAMover.length + 1];
                System.arraycopy(personasAMover, 0, tmp, 0, personasAMover.length);
                personasAMover = tmp;
                personasAMover[personasAMover.length - 1] = camposIzquierda[i].getText();
                System.out.println(camposIzquierda[i].getText());
            }
        }
        System.out.println(personasAMover.length);
        if (personasAMover.length > 0)
        {
     ...
     * @return
     */
    public String[] getPersonasAMover()
    {
        return personasAMover;
    }
    public String getGrupoAMover()
    {
        return grupoAMover;
    }
